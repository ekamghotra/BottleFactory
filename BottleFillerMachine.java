...
* @throws IllegalArgumentException if any of the provided capacities is negative
(less or equal
* to zero)
*/
public BottleFillerMachine(int productionLineCapacity, int fillingCapacity, int
cappingCapacity,
int supplyLineCapacity) {
if (fillingCapacity <= 0 || cappingCapacity <= 0 || productionLineCapacity <= 0
|| supplyLineCapacity <= 0) {
throw new IllegalArgumentException("Capacity of a queue needs to be
positive");
}
  ...
  public void capBottle() {
    ...
    while (!(workDone)) {
      if (supplyLine.isFull()) {
        ...
        System.out
            .println("Number of bottles remaining to fill & cap:" + (--remainingBottlesCount));
      } else {
        // bottle moved from filling queue to capping queue
...
if(numBottles <= 0) {
System.out.println("No bottles to fill! Non-positive number of bottles
" + numBottles + " requested");
return;
}
System.out.println("Bottle Filler Machine to produce " + numBottles + "
bottles.");
...
System.out.println("Bottle " + supplyLine.dequeue().toString() + "!");
System.out.println("Number of bottles remaining to fill & cap:" + (--
...
int productionCapacity = 3, supplyCapacity = 3, fillingCapacity = 2,
cappingCapacity = 2;
BottleFillerMachine factory = new BottleFillerMachine(productionCapacity,
fillingCapacity, cappingCapacity, supplyCapacity);
System.out.println("Bottle Filler Machine Configuration\n"
+ "Production Line("+ productionCapacity+") | Filling Queue("+
fillingCapacity+") | Capping Queue("+cappingCapacity+") |
SupplyLine("+supplyCapacity+")");
